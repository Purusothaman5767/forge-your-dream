...
export default function BudgetBuilder() {
  ...
  const handleSuggest = () => {
    ...
    for (const type of types) {
      const options = productComps.filter(c => c.component_type === type).sort((a, b) => Number(b.price) - Number(a.price));
      const pick = options.find(o => Number(o.price) <= left) || options[options.length - 1];
      if (pick) { result[type] = pick; left -= Number(pick.price); }
    }
  ...
  const handleAddToCart = () => {
    ...
    Object.entries(suggestion).forEach(([type, comp]) => { config[type] = { name: comp.name, price: Number(comp.price) }; });
    addItem({ productId: product.id, productName: product.name, basePrice: Number(product.base_price), configuration: config, totalPrice, image: product.image });
    toast.success('Added to cart!');
    navigate('/cart');
  };

  if (loading) return (
    <div className="container mx-auto px-4 py-12"><div className="h-8 bg-muted rounded w-1/3 mb-8 animate-pulse" /><div className="max-w-xl mx-auto space-y-4">{[1, 2, 3].map(i => <div key={i} className="h-12 bg-muted rounded animate-pulse" />)}</div></div>
  );
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Select value={selectedProduct} onValueChange={v => { setSelectedProduct(v); setSuggestion(null); }}>
              ...
              <SelectContent>
                {products.map(p => (
                  <SelectItem key={p.id} value={p.id}>{p.name} ({formatPrice(Number(p.base_price))})</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium">Budget ({SYMBOL})</label>
            <Input type="number" placeholder="e.g. 96000" value={budget} onChange={e => { setBudget(e.target.value); setSuggestion(null); }} />
          </div>
        </div>
        ...
        <div className="bg-card border rounded-xl p-6 space-y-4">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Suggested within your {SYMBOL}{budget} budget</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {totalPrice <= parseFloat(budget) && (
              <p className="text-xs text-muted-foreground">💰 {formatPrice(parseFloat(budget) - totalPrice)} under budget</p>
            )}
          </div>
